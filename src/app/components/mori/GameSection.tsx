...
import whackAMoleCover from "/Morimori/assets/unnamed(3).jpg";
import whackAMoleMobileCover from "/Morimori/assets/unnamed(3).jpg";
...
const GAMES: Game[] = [
  ...
  },
  {
    ...
    time: "5-10 分",
    image:
      "https://images.unsplash.com/photo-1769490314520-9adb1f4912a4?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxjdXRlJTIwZG9nJTIwaWxsdXN0cmF0aW9ufGVufDF8fHx8MTc3MzE5MTE5NHww&ixlib=rb-4.1.0&q=80&w=1080",
  ...
  },
  {
    ...
    time: "5 分",
    image:
      "https://images.unsplash.com/photo-1578760914717-4b04250b46f9?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHx3b2xmJTIwc2hlZXAlMjBjYXJ0b29uJTIwY3V0ZSUyMGZvcmVzdHxlbnwxfHx8fDE3NzMzMDAxNTF8MA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral",
...
function FloatingTimer() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 flex items-center gap-3 px-5 py-3 rounded-full shadow-xl bg-white border border-stone-100 animate-in fade-in slide-in-from-bottom-4">
      <div
        className={`p-2 rounded-full ${isLow ? "bg-orange-100 text-orange-500" : "bg-emerald-100 text-emerald-500"}`}
      >
        <Hourglass
          className={`w-5 h-5 ${isPlaying ? "animate-pulse" : ""}`}
        />
      ...
      <div className="flex flex-col items-start">
        ...
        <span
          className={`text-xl font-bold font-mono leading-none ${isLow ? "text-orange-500" : "text-stone-700"}`}
        >
            ...
            <Card className="h-full border-none shadow-md overflow-hidden hover:shadow-xl transition-all duration-300">
              <div className="aspect-[4/3] relative">
                ...
                <div className="absolute top-4 left-4">
                  <Badge
                    className={`${game.color} hover:${game.color}`}
                  >
                    {game.age}
                  </Badge>
                </div>
...
function MatchingGame({ onExit }: { onExit: () => void }) {
  ...
  const checkForMatch = (
    ...
    const firstCard = currentCards.find(
      (c) => c.id === firstId,
    );
    const secondCard = currentCards.find(
      (c) => c.id === secondId,
    );
  ...
  return (
    <div className="fixed inset-0 z-50 bg-stone-50 flex flex-col select-none overflow-hidden touch-none bg-emerald-100">
      {/* Header */}
      <div className="flex-none px-4 py-3 bg-white/90 backdrop-blur-md shadow-sm flex justify-between items-center z-20">
        ...
        <div className="flex gap-2">
          <Badge
            variant="outline"
            className="px-3 py-1.5 text-base bg-white border-stone-200 font-mono shadow-sm"
          >
          ...
          <Badge
            variant="outline"
            className="px-3 py-1.5 text-base bg-white border-stone-200 shadow-sm"
          >
            步數: {moves}
          </Badge>
          <Button
            ...
            <div className="grid grid-cols-3 md:grid-cols-4 gap-3 w-full aspect-[3/4] md:aspect-square">
              {cards.map((card) => (
                ...
                >
                  ...
                  >
                    {/* Front (Hidden) */}
                    <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] rounded-xl shadow-sm flex items-center justify-center bg-white border-2 border-emerald-100">
                      <span className="opacity-40 text-emerald-200 text-4xl md:text-5xl font-bold">
                        ?
                      </span>
                    </div>
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <Button
                    variant="ghost"
                    size="sm"
                    className="text-stone-400 hover:text-stone-600 gap-1 rounded-full text-xs"
                  >
          ...
          >
            ...
            <div className="relative z-10 space-y-4">
              ...
              <div className="bg-orange-50 p-4 rounded-xl text-left text-sm text-stone-600 flex gap-3">
                ...
                <p>
                  試著問孩子：「這是什麼動物？」「獅子怎麼叫？」增加語言互動，比輸贏更重要喔。
                </p>
              </div>
              <p className="text-lg text-emerald-600 font-medium pt-2">
                現在，給爸媽一個大大的擊掌！✋
              </p>
            </div>
            ...
            <Button
              onClick={shuffleCards}
              size="lg"
              className="relative z-10 rounded-full bg-emerald-600 hover:bg-emerald-700 text-lg px-8 py-6 w-full shadow-lg shadow-emerald-200"
            >
